...
exports.exportTradingAccountToPDF = async (req, res) => {
  ...
  try {
    // Step 1: Generate trading account data
    const result = await calculateTradingAccountReport({
      db,
      userId,
      financialYear,
    ...
    const data = {
      ...
      debitGroups,
      creditGroups,
      grossResult: grossProfit
        ? { label: 'Gross Profit', amount: grossProfit }
        : grossLoss
          ? { label: 'Gross Loss', amount: grossLoss }
          : null,
      debitTotalAmount,
      debitTotalQuantity,
      creditTotalAmount,
    ...
    const fileName = `tradingAccount_${financialYear}_${Date.now()}.json`;
    ...
    console.log(`✅ Trading Account PDF input uploaded to S3 at ${s3Key}`);
    res.status(200).send({ message: 'Trading Account PDF input successfully uploaded to S3 and tracked.' });

  } catch (err) {
    console.error('❌ Trading Account data generation failed:', err);
    if (exportRecord) {
