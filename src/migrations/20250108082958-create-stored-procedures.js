...
module.exports = {
    async up(queryInterface, Sequelize) {
        // Create stored procedure for archiving old partitions
      ...
      $procedure$
    `);
        // Create stored procedure for pruning old partitions
        await queryInterface.sequelize.query(`
      ...
      `);
        ...
        // Create stored procedure for reindexing active tables
        await queryInterface.sequelize.query(`
        CREATE OR REPLACE PROCEDURE public.reindex_active_tables()
        LANGUAGE plpgsql
        AS $procedure$
        DECLARE
            table_name TEXT;
      ...
      `);
        ...
        // Create stored procedure for reindexing all tables
        await queryInterface.sequelize.query(`
...
AS $procedure$
  ...
  END;
    ...
    async down(queryInterface, Sequelize) {
        // Drop stored procedure for archiving old partitions
        await queryInterface.sequelize.query(`
      DROP PROCEDURE IF EXISTS public.archive_old_partitions;
    `);

        await queryInterface.sequelize.query(`
        DROP PROCEDURE IF EXISTS prune_old_partitions;
      `);

        await queryInterface.sequelize.query(`
        DROP PROCEDURE IF EXISTS public.reindex_active_tables;
      `);

        await queryInterface.sequelize.query(`
        DROP PROCEDURE IF EXISTS public.reindex_all_tables;
      `);
        await queryInterface.sequelize.query(`
        DROP PROCEDURE IF EXISTS public.generate_stock_register;
      `);
    }
};
