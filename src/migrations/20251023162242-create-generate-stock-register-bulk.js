...
WITH RECURSIVE balance_chain AS (
  ...
  SELECT
    ...
    CAST(bc.computed_closing AS numeric(10,4)) AS computed_opening,
    CAST(bc.computed_closing + tsr.quantity + tsr.Received_From_process - tsr.Dispatch_To_process AS numeric(10,4)) AS computed_closing
...
WHERE tsr.id = bc.id;
  ...
  down: async (queryInterface) => {
    await queryInterface.sequelize.query(`
      DROP PROCEDURE IF EXISTS public.generate_stock_register_bulk;
    `);
  }
};
