...
DROP TABLE IF EXISTS temp_stock_ledger;
    ...
    -- Outflow records
          ...
          WHERE type = 8 AND user_id = p_user_id AND financial_year = p_financial_year
            AND entry_date::date BETWEEN v_fin_year_start AND p_end_date;
          ...
          WHERE production_entry_id IS NULL AND user_id = p_user_id
            AND financial_year = p_financial_year AND production_date BETWEEN v_fin_year_start AND p_end_date;
               ...
               -- Average sale rate for process receipts
                    DROP TABLE IF EXISTS temp_sale_rates;
          ...
          WHERE source_type IN ('sale', 'cash_sale')
          GROUP BY item_id;
          ...
          WHERE sl.rate IS NULL
           ...
           DROP TABLE IF EXISTS temp_stock_ledger_opening_snapshot;
           CREATE TEMP TABLE temp_stock_ledger_opening_snapshot AS
SELECT *
FROM temp_stock_ledger
WHERE direction = 'in' AND batch_date <= p_start_date;
    ...
    -- FIFO consumption
          ...
          END LOOP;
           ...
           -- Apply FIFO consumption on snapshot
FOR outflow IN
    SELECT item_id, user_id, financial_year, batch_date, original_qty AS qty, rate
    FROM temp_stock_ledger
    WHERE direction = 'out'
      AND batch_date < p_start_date
    ORDER BY batch_date, batch_id
LOOP
    FOR inflow IN
        SELECT batch_id, remaining_qty
        FROM temp_stock_ledger_opening_snapshot
        WHERE item_id = outflow.item_id
          AND user_id = outflow.user_id
          AND financial_year = outflow.financial_year
          AND remaining_qty > 0
        ORDER BY batch_date, batch_id
    LOOP
        EXIT WHEN outflow.qty <= 0;
        consume_qty := LEAST(outflow.qty, inflow.remaining_qty);
        UPDATE temp_stock_ledger_opening_snapshot
        SET remaining_qty = remaining_qty - consume_qty
        WHERE batch_id = inflow.batch_id;
        outflow.qty := outflow.qty - consume_qty;
    END LOOP;
...
-- FIFO valuation for closing stock
WITH fifo_closing_batches AS (
    ...
    SELECT sr.item_id, 0 AS rate, 0 AS remaining_qty, NULL AS batch_date, 0 AS cumulative_qty
    FROM stock_register sr
    WHERE sr.user_id = p_user_id
      AND sr.financial_year = p_financial_year
      AND sr.entry_date = p_end_date
      AND sr.item_id NOT IN (
          SELECT DISTINCT item_id FROM temp_stock_ledger WHERE direction = 'in'
      )
),
eligible_closing_batches AS (
...
closing_negative_rates AS (
    SELECT item_id,
           closing_balance,
           (
             SELECT rate
             FROM fifo_closing_batches
...
-- Final insert into stock_valuation
INSERT INTO stock_valuation (
    item_id, user_id, financial_year, start_date, end_date,
...
SELECT
    ...
    p_end_date,
    CASE
      WHEN cb.closing_balance < 0 THEN cnr.closing_balance * cnr.rate
      ELSE SUM(cb.qty_to_value * cb.rate)
    END AS closing_stock_valuation
FROM eligible_closing_batches cb
LEFT JOIN closing_negative_rates cnr ON cb.item_id = cnr.item_id
GROUP BY cb.item_id, cb.closing_balance, cnr.rate, cnr.closing_balance;

-- FIFO valuation for opening stock
WITH fifo_opening_batches AS (
    SELECT item_id, rate, remaining_qty, batch_date,
           SUM(remaining_qty) OVER (PARTITION BY item_id ORDER BY batch_date) AS cumulative_qty
    FROM temp_stock_ledger_opening_snapshot
    ...
    WHERE sr.user_id = p_user_id
      AND sr.financial_year = p_financial_year
      AND sr.entry_date = p_start_date
      AND sr.item_id NOT IN (
          SELECT DISTINCT item_id FROM temp_stock_ledger_opening_snapshot
      )
),
eligible_opening_batches AS (
    SELECT fb.item_id, fb.rate, fb.remaining_qty, fb.cumulative_qty, sr.opening_balance,
           CASE
             WHEN sr.opening_balance <= 0 THEN sr.opening_balance
             WHEN fb.cumulative_qty <= sr.opening_balance THEN fb.remaining_qty
             ELSE sr.opening_balance - (fb.cumulative_qty - fb.remaining_qty)
           END AS qty_to_value
    FROM fifo_opening_batches fb
    JOIN (
...
opening_negative_rates AS (
    SELECT item_id,
           opening_balance,
           (
             SELECT rate
             FROM fifo_opening_batches
             WHERE item_id = sr.item_id
             ORDER BY batch_date DESC, rate DESC
             LIMIT 1
           ) AS rate
...
UPDATE stock_valuation sv
SET opening_stock_valuation = COALESCE(ov.opening_stock_valuation, onr.opening_balance * onr.rate)
FROM (
    SELECT item_id,
           SUM(qty_to_value * rate) AS opening_stock_valuation
    FROM eligible_opening_batches
    GROUP BY item_id
) ov
LEFT JOIN opening_negative_rates onr ON ov.item_id = onr.item_id
WHERE sv.item_id = ov.item_id
  AND sv.user_id = p_user_id
  AND sv.financial_year = p_financial_year
  AND sv.start_date = p_start_date
  AND sv.end_date = p_end_date;
